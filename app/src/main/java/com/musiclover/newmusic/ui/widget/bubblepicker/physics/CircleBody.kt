package com.musiclover.newmusic.ui.widget.bubblepicker.physics

import android.util.Log
import android.view.animation.Interpolator
import android.view.animation.OvershootInterpolator
import com.musiclover.newmusic.ui.widget.bubblepicker.sqr
    ...
    fun step(deltaInSecond : Float) {
        ...
        when (state) {
            ...
            STATE_MOTION_ENHANCE -> {
                ...
                if(animatedValue>1f) animatedValue = 1f
                currentRatio = PhysicsEngine.interpolate(
                    1f,
                    ENHANCE_VALUE,
                    interpolator.getInterpolation(animatedValue)
                )
                updateSize()
                if(animatedValue ==1f) endMotion()
            }
