package com.musiclover.newmusic.ui.widget.soundfile;
 ...
 */
public class WavFileDescriptor {
    ...
    private AssetFileDescriptor file;                        // File that will be read from or written to
    ...
    public static WavFileDescriptor openWavFile(AssetFileDescriptor file) throws IOException, WavFileException {
        // Instantiate new Wavfile and store the file reference
        WavFileDescriptor wavFile = new WavFileDescriptor();
        wavFile.file = file;

        // Create a new file input stream for reading file data
        wavFile.iStream = file.createInputStream();
        ...
        if (file.getLength() != chunkSize + 8) {
            throw new WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + file.getLength() + ")");
        }
